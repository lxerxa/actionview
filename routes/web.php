...
Route::group(['prefix' => $api_prefix . '/project/{project_key}', 'middleware' => ['can', 'checkProjectStatus', 'privilege:view_project']], function () {
    ...
    Route::middleware(['api','privilege:upload_file'])->group(function () {
        Route::post('file', 'FileController@upload');
        // Route::post('file', 'FileController@upload')->withoutMiddleware([\Illuminate\Foundation\Http\Middleware\VerifyCsrfToken::class]);
    }); 

    Route::get('file/{id}/thumbnail', 'FileController@downloadThumbnail');
    Route::middleware('privilege:download_file')->group(function () {
        Route::get('file/{id}/{name?}', 'FileController@download');
    }); 
